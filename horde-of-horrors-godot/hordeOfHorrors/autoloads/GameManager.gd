...
func change_environment(new_environment_path: String) -> void:
	# Assuming current scene is MainGame and environments are children of it or loaded directly
	# This will need to be refined based on actual scene structure (e.g., loading new MainGame scene)
	get_tree().change_scene_to_file(new_environment_path)
